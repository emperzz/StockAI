...
class AKShareAdapter(BaseDataAdapter):
    ...
    def _initialize(self) -> None:
        ...
        self.china_index_exchange_code = {
            Exchange.SSE: 'sh',
            Exchange.SZSE: 'sz',
            Exchange.BSE: 'bj'
        }
    ...
    def _get_field_names(self, df: pd.DataFrame, exchange: Exchange) -> Dict[str, str]:

        return {
            'ticker' : self._get_field_name(df, "code", exchange),
            ...
            'pre_close_field' : self._get_field_name(df, 'pre_close', exchange),
            'price_field' : self._get_field_name(df, 'price', exchange),
            ...
            'amount_field' : self._get_field_name(df, "amount", exchange),
    ...
    def get_trading_calendar(self, exchange: str) -> List[datetime]:
        """Get avaliable trading days 

        Args:
            exchange: exchange symbol, e.g SSE, NADSQ

        Returns:
            list of avaliable trading dates
        """
        if exchange in [Exchange.SSE.value, Exchange.SZSE.value, Exchange.BSE.value]:
            try:
                df = ak.tool_trade_date_hist_sina()
            ...
            logger.warning(f"Unsupport exchange: {exchange}")

    def get_real_time_market(self, exchange: Optional[str] = None) -> Optional[List[AssetPrice]]:
        """Get the latest real-time price of china stock market.

        Args:
            exchange: e.g SSE,SZSE
        ...
        try:
            if exchange in [Exchange.SSE.value, Exchange.SZSE.value, Exchange.BSE.value]:
                for attempt in Retrying(
                    ...
                    with attempt:
                        df = ak.stock_zh_a_spot() #stock_zh_a_spot_em
            elif exchange in [Exchange.NASDAQ, Exchange.NYSE, Exchange.AMEX]:
            ...
            data_time = tdf[pd.to_datetime(tdf.trade_date) <= datetime.now()].trade_date.values[-1]
            
            return self._convert_market_df_to_prices(df, data_time)
        except Exception as e:
            logger.error(
                f"Error getting real-time price for market {exchange}: {e}", exc_info=True
            )
            return None


    def _convert_market_df_to_prices(self, df: pd.DataFrame, data_time: Union[datetime, str] ) -> Optional[List[AssetPrice]]:
        """
        """
        prices = []
        ...
        try:
            ...
            field_names = self._get_field_names(df, exchange)
            time_field = self._get_field_name(df, 'time', exchange)

            # Validate required fields
            if not field_names['close_field'] or not field_names['ticker_field']:
                logger.error(
                    f"Missing required fields in DataFrame. close_field={field_names['close_field']},ticker_field = {field_names['ticker_field']}"
                )
                return None
            ...
            for _, row in df.iterrows():
                try:
                    # Parse date
                    date_str = str(data_time) + " " + row[time_field]
                    ...
                    price = self._convert_row_to_price(row, field_names, currency = currency, timestamp = timestamp)
                    prices.append(price)
    ...
    def get_real_time_price(self, ticker: str) -> Optional[AssetPrice]:
        """Get the latest real-time price via unified intraday pipeline.
        ...
        try:
            ...
            prices = self._get_intraday_prices(
                ticker=ticker,
    ...
    def _get_intraday_prices(
        ...
        try:
            ...
            # Parse exchange and symbol from ticker
            # exchange_str, symbol = ticker.split(":", 1)
            # try:
            #     exchange = Exchange(exchange_str)
            # except ValueError:
            ...
            # Convert to AKShare format for data source specific needs
            source_ticker = self.convert_to_source_ticker(ticker)
    ...
    def _convert_intraday_df_to_prices(
        ...
        try:
            ...
            # Validate required fields
            if not field_names['time_field'] or not field_names['close_field']:
                logger.error(
                    f"Missing required fields in DataFrame. time_field={field_names['time_field']}, close_field={field_names['close_field']}"
                )
                return []
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    price = self._convert_row_to_price(row, field_names, currency = currency, ticker = ticker)
                    prices.append(price)
    ...
    def convert_to_source_ticker(self, internal_ticker: str) -> str:
        ...
        try:
            ...
            asset_type = self._check_asset_type(internal_ticker)

            # Handle Indexs
            if asset_type == AssetType.INDEX:
                # TODO: currently check_asset_type doesn't us market
                if exchange_enum in [Exchange.NASDAQ, Exchange.NYSE, Exchange.AMEX]:
                    return f"{self.us_index_exchange_code}.{symbol}"
                elif exchange_enum in [Exchange.SSE, Exchange.SZSE, Exchange.BSE]:
                    return f"{self.china_index_exchange_code[exchange_enum]}{symbol}"
    ...
    def convert_to_internal_ticker(
        ...
        if len(st_lower) >= 8 and st_lower[:2] in {"sh", "sz", "bj"}:
            prefix = st_lower[:2]
            symbol = source_ticker[2:]
            if symbol.isdigit() and len(symbol) == 6:
                if prefix == "sh":
                    return f"{Exchange.SSE.value}:{symbol}"
                if prefix == "sz":
                    return f"{Exchange.SZSE.value}:{symbol}"
                if prefix == "bj":
                    return f"{Exchange.BSE.value}:{symbol}"
